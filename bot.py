...
class BybitBot:
    ...
    def __init__(self, api_key, api_secret, demo):
        if type(self) is BybitBot:
            raise TypeError("BybitBot is an abstract class and cannot be instantiated directly.")
    ...
    def _enable_collateral_coin(self, coin):
        r = self.session.set_collateral_coin(
            coin=coin,
            collateralSwitch="ON"
        )
        assert r['retCode'] == 0, \
            f'Error enabling collateral coin: {r["retMsg"]}'
...
class GridBot(BybitBot):
    ...
    @property
    def settleCoin(self):
        if hasattr(self, '_settleCoin'):
            return self._settleCoin
        
        self._settleCoin = self.instruments_info['settleCoin']
        return self._settleCoin
    ...
    def qty(self):
        ...
        if self.category == 'linear':
            qty = self.total_investment * self.leverage \
                / ((grids.sum() - grids[mid_idx]) * (1 + self.INITIAL_MARGIN) + larger_portion.sum() * self.MAINTENANCE_MARGIN)
        else:
            qty = self.total_investment * self.leverage \
                / (((1 / grids).sum() - 1 / grids[mid_idx]) * (1 + self.INITIAL_MARGIN) + (1 / larger_portion).sum() * self.MAINTENANCE_MARGIN)
...
if __name__ == '__main__':
    ...
    session = HTTP(api_key=api_key, api_secret=api_secret, demo=demo)
    category = 'inverse'
    symbol = 'BTCUSD'
    price = float(session.get_tickers(category=category, symbol=symbol,)['result']['list'][0]['markPrice'])
    spread_pct = 0.01
    low = price * (1 - spread_pct / 2)
    high = price * (1 + spread_pct / 2)
    ngrid = 100
    grid_spread = price * spread_pct / ngrid
    ...
    bot = GridBot(
        ...
        low_sl = round(low - grid_spread, 1),
        high_sl = round(high + grid_spread, 1),
        update_period = 0,
        logger_level = 'DEBUG',
    )
